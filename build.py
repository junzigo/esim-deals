...
def build(config_path=None, output=None, data_path=None):
    ...
    pages={
      ...
      'privacy':('Privacy','<h2>A simple static site.</h2><p>This site has no account signup, advertising tracker or analytics script. Copying a coupon uses your browser clipboard only when you click the copy button. No coupon-copy data is sent to us.</p><p>Cloudflare hosts the site and may process request information to deliver and secure it. External provider sites have their own privacy and cookie policies.</p>')}
